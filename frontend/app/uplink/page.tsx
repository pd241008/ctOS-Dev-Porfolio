...
export default function UplinkPage() {
  ...
  const contactLinks = [
    { label: "GITHUB_DATABASE", val: "github.com/pd241008", href: "https://github.com/pd241008", icon: "⚡" },
    { label: "LINKEDIN_UPLINK", val: "linkedin.com/in/pd241008", href: "https://linkedin.com/in/pd241008", icon: "🌐" },
    { label: "DIRECT_NODE_PING", val: "+91 9923464748", href: "tel:+919923464748", icon: "🛰️" }
  ];
    ...
    <div className="animate-in fade-in zoom-in-95 duration-500 max-w-6xl mx-auto py-12 px-4 md:px-8 space-y-12 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-sm blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
          <SystemCard title="TERMINAL_COMMAND // INIT_EMAIL_PROTOCOL">
            <div className="p-8 bg-zinc-950 flex flex-col min-h-[550px]">
              ...
              {emailState.status === "SUCCESS" ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8 animate-in zoom-in-95">
                  <div className="w-20 h-20 rounded-full border-2 border-green-500 flex items-center justify-center text-3xl text-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)] bg-green-950/20">✓</div>
                  <div className="space-y-3">
                    <h3 className="text-2xl font-black text-white uppercase tracking-widest">Sent_Successfully</h3>
                    <p className="text-zinc-500 text-[11px] font-mono uppercase tracking-widest max-w-sm leading-relaxed">
                      Your payload has been successfully routed to the operator&apos;s node.
                    </p>
                  </div>
                  <div className="w-full bg-zinc-900/50 p-4 font-mono text-[9px] text-zinc-500 text-left">
                    {logs.map((log, i) => (
                      <div key={i} className="mb-0.5 text-green-500/70">{log}</div>
                    ))}
                  </div>
                  <button onClick={() => setEmailState({ subject: "", message: "", status: "IDLE" })} className="text-[10px] text-purple-500 font-black hover:text-white transition-colors underline underline-offset-4 tracking-[0.2em]">
                    [ INIT_NEW_PROTOCOL ]
                  </button>
                </div>
              ) : (
                <div className="flex-1 flex flex-col">
                  ...
                  <form onSubmit={handleSendMail} className={`space-y-6 flex-1 flex flex-col ${emailState.status === "SENDING" ? "opacity-30 pointer-events-none" : ""}`}>
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-zinc-500 font-black uppercase tracking-widest ml-1">Subject_Line</label>
                      <input 
                        required
                    ...
                    <div className="flex-1 flex flex-col space-y-1.5">
                      <label className="text-[10px] text-zinc-500 font-black uppercase tracking-widest ml-1">Secure_Payload</label>
                      <textarea 
                        required
                        ...
                        className="flex-1 w-full bg-zinc-900/50 border border-zinc-800 p-4 text-zinc-200 font-mono text-xs focus:border-purple-500 outline-none transition-all resize-none placeholder:text-zinc-700 min-h-[200px]"
                      />
                    </div>
                    ...
                    >
                      {emailState.status === "SENDING" ? "Transmitting..." : "Execute_Protocol"}
                    </button>
                  </form>
                </div>
              )}
           ...
           <div className="p-8 bg-zinc-900/40 border-2 border-zinc-800 relative group overflow-hidden">
             ...
             <div className="relative space-y-6">
                ...
                <p className="text-zinc-300 text-lg font-bold italic leading-relaxed tracking-wide">
                  &quot;Looking to collaborate on scalable backend systems, AI integrations, or open-source tooling? The network is open.&quot;
                </p>
             </div>
           </div>
              ...
              {contactLinks.map(link => (
                <a key={link.label} href={link.href} target="_blank" rel="noreferrer" 
                  ...
                  <div className="flex items-center gap-6">
                    <span className="text-3xl grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all">{link.icon}</span>
                    <div className="space-y-1 relative z-10">
                      <div className="text-[10px] text-zinc-600 font-black uppercase tracking-[0.2em] group-hover:text-purple-400 mb-1">{link.label}</div>
                      <div className="text-[14px] text-zinc-300 font-bold group-hover:text-white transition-colors underline decoration-purple-500/30 group-hover:decoration-purple-500 underline-offset-8">{link.val}</div>
                    </div>
                  </div>
                  <span className="text-zinc-800 group-hover:text-purple-500 transition-colors font-black text-xl">»</span>
                </a>
              ))}
           </div>
        </div>
      ...
      <div className="flex justify-center pt-20 border-t border-zinc-800">
        <button onClick={() => router.back()} className="px-8 py-2 text-[11px] font-black uppercase tracking-[0.3em] bg-zinc-950 border border-zinc-800 text-zinc-600 hover:border-purple-500/50 hover:text-purple-400 transition-all">
          ⋘ Return_To_Archives
        </button>
      </div>
    </div>
